...
(defn generate-prompt-manifest
  "Generate Clojure source file listing all workflow and category prompts.

  Scans resources/prompts/ and resources/category-prompts/ directories and creates
  src/mcp_tasks/generated_workflows.clj containing vectors of prompt names
  (without .md extension).

  This approach embeds the prompt lists directly in compiled code, which works
  reliably in both JAR and GraalVM native images without requiring runtime
  resource loading."
  [_opts]
  (let [prompts-dir (io/file "resources/prompts")
        workflow-files (->> (file-seq prompts-dir)
                            ...
                            sort
                            vec)
        categories-dir (io/file "resources/category-prompts")
        category-files (->> (file-seq categories-dir)
                            (filter #(and (.isFile %)
                                          (clojure.string/ends-with? (.getName %) ".md")))
                            (map #(clojure.string/replace (.getName %) #"\.md$" ""))
                            sort
                            vec)
        output-file (io/file "src/mcp_tasks/generated_workflows.clj")
        content (str "(ns mcp-tasks.generated-workflows\n"
                     ...
                     "  " (pr-str workflow-files) ")\n\n"
                     "(def builtin-categories\n"
                     "  \"Set of built-in category prompt names (without .md extension).\"\n"
                     "  " (pr-str (set category-files)) ")\n")]
    (spit output-file content)
    (println (format "Generated prompt lists: %d workflows, %d categories in %s"
                     (count workflow-files)
                     (count category-files)
                     (.getPath output-file)))
    {:workflows (count workflow-files)
     :categories (count category-files)}))
