...
{% if cli %}
Check if `.mcp-tasks/prompt-optimisation.edn` exists:
- If exists: Read the state file
- If not: Initialize with `{:last-run nil :processed-story-ids #{} :modifications []}`
{% else %}
...
{% if cli %}
Read `.mcp-tasks/complete.ednl` and filter for stories that:
- Have `:type :story`
- Have non-empty `:session-events`
- Are not in `:processed-story-ids` from state
{% else %}
...
{% if cli %}
For each story, categorize session events into:
- **Compactions** (`:event-type :compaction`): Indicate context limits reached
- **Corrections** (`:event-type :user-prompt` matching correction patterns):
  Indicate Claude deviated from expected behavior
- **Restarts** (`:event-type :session-start` after first): Indicate workflow
  interruptions
{% else %}
...
{% if cli %}
Format findings as markdown grouped by type. Number each finding sequentially
for user selection.
{% else %}
...
{% if cli %}
   ...
   - Show the diff to the user before saving
{% else %}
...
{% if cli %}
Write updated state to `.mcp-tasks/prompt-optimisation.edn`:
- Set `:last-run` to current ISO-8601 timestamp
- Add all analyzed story IDs to `:processed-story-ids`
- Append any new modifications to `:modifications`
{% else %}
