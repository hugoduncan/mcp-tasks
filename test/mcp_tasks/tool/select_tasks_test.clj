...
(defn- write-tasks-ednl
  ...
  [tasks]
  (h/write-ednl-test-file "tasks.ednl" tasks))
...
(deftest select-tasks-returns-multiple-tasks
  ...
  (testing "select-tasks returns multiple matching tasks"
    ;; Add three tasks
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task One"})
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task Two"})
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task Three"})
...
(deftest select-tasks-limit-parameter
  ...
  (testing "select-tasks :limit parameter"
    ;; Add five tasks
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task 1"})
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task 2"})
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task 3"})
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task 4"})
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task 5"})

    (testing "returns up to limit tasks"
      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:category "test" :limit 3})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (false? (:isError result)))
        (is (= 3 (count (:tasks response))))
        (is (= 3 (get-in response [:metadata :count])))
...
(deftest select-tasks-unique-constraint
  ...
  (testing "select-tasks :unique constraint"
    (testing "returns task when exactly one matches"
      (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Unique Task"})

      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:category "test" :unique true})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (false? (:isError result)))
    ...
    (testing "returns empty when no matches"
      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:category "nonexistent" :unique true})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (false? (:isError result)))
...
(deftest select-tasks-validation-errors
  ...
  (testing "select-tasks validation errors"
    (testing "non-positive limit returns error"
      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:limit 0})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (false? (:isError result)))
        (is (contains? response :error))
    ...
    (testing "negative limit returns error"
      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:limit -5})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (false? (:isError result)))
        (is (contains? response :error))
        (is (str/includes? (:error response) "positive integer"))))
    ...
    (testing "limit > 1 with unique true returns error"
      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:limit 5 :unique true})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (false? (:isError result)))
        (is (contains? response :error))
...
(deftest select-tasks-empty-results
  ...
  (testing "select-tasks empty results"
    (let [result (#'sut/select-tasks-impl (h/test-config) nil {:category "nonexistent"})
          response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
      (is (false? (:isError result)))
      (is (= [] (:tasks response)))
      (is (= 0 (get-in response [:metadata :count])))
...
(deftest select-tasks-metadata-accuracy
  ...
  (testing "select-tasks metadata accuracy"
    ;; Add tasks
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task A"})
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task B"})
    (#'add-task/add-task-impl (h/test-config) nil {:category "other" :title "Task C"})

    (testing "metadata reflects filtering and limiting"
      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:category "test" :limit 1})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (= 1 (get-in response [:metadata :count])))
    ...
    (testing "metadata when not limited"
      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:category "test" :limit 10})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
...
(deftest select-tasks-type-filter
  ...
  (testing "select-tasks :type filter"
    ...
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Regular task" :type "task"})
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Bug fix" :type "bug"})
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "New feature" :type "feature"})
    (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Another task" :type "task"})

    (testing "filters by type task"
      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:type "task"})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (false? (:isError result)))
    ...
    (testing "filters by type bug"
      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:type "bug"})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (false? (:isError result)))
        (is (= 1 (count (:tasks response))))
    ...
    (testing "filters by type feature"
      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:type "feature"})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (false? (:isError result)))
        (is (= 1 (count (:tasks response))))
    ...
    (testing "combines type filter with category filter"
      (#'add-task/add-task-impl (h/test-config) nil {:category "other" :title "Other bug" :type "bug"})

      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:category "test" :type "bug"})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (false? (:isError result)))
        (is (= 1 (count (:tasks response))))
        (is (= "Bug fix" (get-in response [:tasks 0 :title])))))))
...
(deftest select-tasks-status-filter
  ...
  (testing "select-tasks :status filter"
    (testing "filters by status open (default behavior)"
      ...
      (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Open task 1"})
...
(deftest select-tasks-status-in-progress
  ...
  (testing "select-tasks :status filter"
    (testing "filters by status in-progress"
      ...
      (let [result (#'sut/select-tasks-impl (h/test-config) nil {:status "in-progress"})
            response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
        (is (false? (:isError result)))
        (is (= 2 (count (:tasks response))))
...
(deftest select-tasks-task-id-filter
  ...
  (testing "select-tasks :task-id filter"
    ;; Add tasks
    (let [task1 (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task One"})
          task1-data (json/read-str (get-in task1 [:content 1 :text]) :key-fn keyword)
          task1-id (get-in task1-data [:task :id])

          task2 (#'add-task/add-task-impl (h/test-config) nil {:category "test" :title "Task Two"})
          task2-data (json/read-str (get-in task2 [:content 1 :text]) :key-fn keyword)
          task2-id (get-in task2-data [:task :id])

          task3 (#'add-task/add-task-impl (h/test-config) nil {:category "other" :title "Task Three"})
      ...
      (testing "filters by task-id to return single task"
        (let [result (#'sut/select-tasks-impl (h/test-config) nil {:task-id task1-id})
              response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
          (is (false? (:isError result)))
          (is (= 1 (count (:tasks response))))
      ...
      (testing "returns empty when task-id does not exist"
        (let [result (#'sut/select-tasks-impl (h/test-config) nil {:task-id 99999})
              response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
          (is (false? (:isError result)))
          (is (= 0 (count (:tasks response))))
          (is (= 0 (get-in response [:metadata :count])))
      ...
      (testing "combines task-id filter with category filter"
        ...
        (let [result (#'sut/select-tasks-impl (h/test-config) nil {:task-id task3-id :category "test"})
              response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
          (is (false? (:isError result)))
          (is (= 0 (count (:tasks response)))))

        ;; Filtering by task3-id and category "other" should return task 3
        (let [result (#'sut/select-tasks-impl (h/test-config) nil {:task-id task3-id :category "other"})
              response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
          (is (false? (:isError result)))
          (is (= 1 (count (:tasks response))))
      ...
      (testing "task-id filter works with unique constraint"
        (let [result (#'sut/select-tasks-impl (h/test-config) nil {:task-id task2-id :unique true})
              response (json/read-str (get-in result [:content 0 :text]) :key-fn keyword)]
          (is (false? (:isError result)))
          (is (= 1 (count (:tasks response))))
...
;; Tests from story_tools_test.clj
  ...
  (testing "select-tasks with parent-id filter"
    (testing "returns first incomplete child task"
      (let [story {:id 1
        ...
        (let [config {:base-dir h/*test-dir* :use-git? false}
